...
def get_result_dictionary(csv):
    data = {}

    algorithms = []
    for i in range(len(csv)):
        # load algorithm names in line 0
        if i == 0:
            for name in csv[0]:
            ...
            for j in range(len(csv[i])):
                data[algorithms[j]].append(float(csv[i][j]))
...
def main():
    ...
    for file_name in file_names:
        result[file_name] = get_result_dictionary(read_text_file(file_path + file_name))

    df = pd.DataFrame(result["curl_lan.csv"], columns=algorithms)
