...
在TCP的具体实现中，当接收方收到了一个不按顺序抵达的数据包时，应当立刻向发送方回复一个重复的确认，以通知发送方此数据包乱序抵达并提示应当收到的数据包的编号。造成数据包乱序抵达的原因可以是网络出现了拥塞、网络调换了数据包的顺序[22]又或者是网络复制了一个数据包等原因。对于发送方而言，收到重复的确认就可以认为是网络出现了拥塞。
