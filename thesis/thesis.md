...
|WAN 1|中国深圳阿里云机房|暨南大学数据中心|由阿里云方面共享网络限制，实际测试中的最高平均速度约为1.2Mbps|
|WAN 2|美国Portland|中国北京阿里云机房|由阿里云方面的共享网络限制，实际测试中的最高平均速度约为15Mbps|
...
在对每一个场景进行全面测试之前，都会通过iperf3来对目标场景进行长时间的连续测试，以求全面地了解在该场景下用来作为基准的CUBIC（有线网络）算法和Westwood（无线网络）在RTT、带宽和重传计数等三个方面的性能表现。在基准测试中获得的RTT、带宽和重传计数等，其平均值将会作为基准线出现在之后的测试的结果图及其分析中。在两类网络上的详细设定上的差异如下表：
