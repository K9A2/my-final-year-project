...
在对每一个场景进行全面测试之前，都会通过iperf3来对目标场景进行长时间的连续测试，以求全面地了解在该场景下用来作为基准的CUBIC算法在RTT、带宽和重传计数三个方面的性能表现。在基于有线网络的测试场景中，基准测试时间为24小时；在基于无线网络的测试场景中，基准测试时间为1小时。在基准测试中获得的RTT、带宽和重传计数等，将会作为基准线出现在之后的测试的结果图及其分析中。

#### 3.4.2 TCP性能测试

在基准测试之后，将使用iperf3对所有场景中的TCP拥塞控制算法进行详细测试。主要的测试项目是在该场景下，由特定TCP拥塞控制算法控制下的RTT与带宽大小。在此测试过程中，将引入其他TCP流，以考察该TCP拥塞控制算法在有竞争的情况下的性能表现。除了发送测试数据的主机之外，其余主机的各项设定固定。

#### 3.4.3 Web应用测试

在完成TCP性能测试之后，将使用curl进行文件下载测试。在此类测试之中，将在测试主机上使用Apache建立服务器，并在远程主机上下载服务器中的文件。重复下载五十次，并计算所有操作的下载速度平均值。

#### 3.4.4 公平性测试

在完成应用测试之后，将在Mininet中按照基准测试中获得的网络特性参数（包括RTT、带宽和丢包率等）设定虚拟网络的特性参数，并按照哑铃型拓扑组建虚拟网络。拓扑中的两台发送端主机通过两台交换机组成的单瓶颈链路向处于瓶颈另一端的接收主机分别使用iperf3发送数据。
